...
Discord/LLM シグナルを起点としたトレード実行と、ブローカー連携を行うバックエンド。**実売買の主軸は moomoo 証券（OpenAPI + OpenD）**。ローカル検証用に `paper` ブローカーも利用できる。詳細は [backend/README.md](backend/README.md) を参照。
...
- **ブローカー経由**  
  - moomoo OpenAPI 等: ヒストリカル・バー API で取得（実装はデータソースに依存）。  
- **取得単位**  
...
| フェーズ | 内容 |
|----------|------|
| **Phase 1** | 過去データ取得パイプラインの設計・実装（moomoo など 1 ソースから開始）。ストレージスキーマとジョブの追加。 |
